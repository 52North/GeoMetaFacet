...
Exhibit.ListFacet.prototype.clearAllRestrictions = function() {
    ...
    //if (this._cache._expression._rootNode._segments[0].property == organizationFacet)    
...
//--------------------------------------------------------------------------------    
    ...
    var selectedTopics, selectedDatatypes, selectedOrganizations, selectedScenarios, selectedBoundingboxes, selectedHierarchylevelnames;
    
    if (!resetAllBool) {
	    //check selected facet value @see exhibitOutputFunctions.js
	    selectedTopics = getTopicSelection(); 
	    selectedDatatypes = getDatatypeSelection(); 
	    selectedOrganizations = getOrganizationSelection();
	    selectedScenarios = getScenarioSelection();
	    selectedBoundingboxes = getBoundingboxSelection(); 
	    selectedHierarchylevelnames = getHierarchylevelnameSelection();
    }
    
    var facetName = this._cache._expression._rootNode._segments[0].property;
    
    if (facetName == topicFacet) {    	
    	if (selectedDatatypes != "service") { //service metadata do not have a topic category
	    	entries = [];
	    
	    	if (resetAllBool) { 
	    		entries = getInitValues(topicFacet);
	    	} else {	   		    	
		    	var topicArray = httpGet(findAllTopics + "/" + selectedHierarchylevelnames + "/" + selectedDatatypes + "/" + selectedOrganizations + "/" + selectedScenarios + "/" + selectedBoundingboxes);
		    	var topicNames = "";topicArray;  
		    	for (var i = 0; i < topicArray.length; i++) {
		    		if (i < topicArray.length - 1) 
		    			topicNames += topicArray[i] + "..";
		    		else
		    			topicNames += topicArray[i];
		    	}  
		    	
		    	if (topicNames != "") {
			    	var topicCounts = httpGet(countAllTopiccategories + "/" + selectedHierarchylevelnames + "/" + topicNames + "/" + selectedDatatypes + "/" + selectedOrganizations + "/" + selectedScenarios+ "/" + selectedBoundingboxes);	
			    	 
			    	for (var i = 0; i < topicArray.length; i++) {
			    							//facet value			//no. of this facet value  
			    		entries.push({ value: topicArray[i], count: topicCounts[i].count });
			    	} 
			    	
			    	if (globalCount == 0) {
			    		//@see exhibitOutputFunctions
			    		setInitValues(topicFacet, entries);
			    	}	
		    	}
	    	}
    	}
    } else if (facetName == datatypeFacet) {
    	entries = [];	
    	
    	if (resetAllBool) { 
    		entries = getInitValues(datatypeFacet);
    	} else {		
	    	var datatypeArray = httpGet(findAllDatatypes + "/" + selectedHierarchylevelnames + "/" + selectedTopics + "/" + selectedOrganizations + "/" + selectedScenarios+ "/" + selectedBoundingboxes);
	    	var datatypeNames = "";//datatypeArray;//"";
	    	
	    	for (var i = 0; i < datatypeArray.length; i++) {
	    		if (i < datatypeArray.length - 1) 
	    			datatypeNames += datatypeArray[i] + ";";
	    		else
	    			datatypeNames += datatypeArray[i];
	    	}
	    	 
	    	if (datatypeNames != "") {
		    	var datatypeCounts = httpGet(countAllDatatypes + "/" + selectedHierarchylevelnames + "/" + selectedTopics + "/" + datatypeNames + "/" + selectedOrganizations + "/" + selectedScenarios+ "/" + selectedBoundingboxes);
		    	
		    	for (var i = 0; i < datatypeArray.length; i++) {
		    							//facet value			//no. of this facet value		
		    		entries.push({ value: datatypeArray[i], count: datatypeCounts[i].count });    		
		    	} 
		    	
		    	if (globalCount == 0) {
		    		//@see exhibitOutputFunctions
		    		setInitValues(datatypeFacet, entries);
		    	}
	    	}
    	}
    } else if (facetName == organizationFacet) {
    	entries = [];
    	
    	if (resetAllBool) { 
    		entries = getInitValues(organizationFacet); 
    		resetAllBool = false;
    	} else {
	    	var organizationArray = httpGet(findAllOrganizations + "/" + selectedHierarchylevelnames + "/" + selectedTopics + "/" + selectedDatatypes + "/" + selectedScenarios + "/" + selectedBoundingboxes);   	    	
	    	var orgaNames = "";//organizationArray; //"";
	    	
	    	for (var i = 0; i < organizationArray.length; i++) {
	    		if (i < organizationArray.length - 1) 
	    			orgaNames += organizationArray[i] + ".."; 
	    		else
	    			orgaNames += organizationArray[i];
	    	} 
	    	 
	    	if (orgaNames != "") {
		    	var organizationCounts = httpGet(countAllOrganizations + "/" + selectedHierarchylevelnames + "/" + selectedTopics + "/" + selectedDatatypes + "/" + orgaNames + "/" + selectedScenarios+ "/" + selectedBoundingboxes);
		    	
		    	for (var i = 0; i < organizationArray.length; i++) {	    		
		    		 if (organizationArray[i] == organizationCounts[i].orga) 
		    							//facet value			//no. of this facet value				
		    			entries.push({ value: organizationArray[i], count: organizationCounts[i].count });	    		 
		    	} 
		    	
		    	if (globalCount == 0) {
		    		//@see exhibitOutputFunctions
		    		setInitValues(organizationFacet, entries);
		    	}
	    	}
	    	globalCount++;
    	}
    } else if (facetName == boundingboxFacet) {
    	var boxArray = httpGet(findAllBoundingboxes + "/" + selectedTopics + "/" + selectedDatatypes + "/" + selectedOrganizations + "/" + selectedScenarios + "/" + selectedHierarchylevelnames);   	    	
    	var boxNames = "";
    	...
    	if (boxNames != "") {
	    	var boxCounts = httpGet(countAllBoundingboxes + "/" + selectedHierarchylevelnames + "/" + selectedTopics + "/" + selectedDatatypes + "/" + selectedOrganizations + "/" + selectedScenarios + "/" + boxNames);
	    	
	    	for (var i = 0; i < boxArray.length; i++) {	  
	    							//facet value			//no. of this facet value				
	    			entries.push({ value: boxArray[i], count: boxCounts[i].count });	    		 
	    	} 
