...
//CH
var projectUpdateFlag = true;
var scenarioUpdateFlag = true;
...
//CH: neu
Exhibit.HierarchicalFacet.prototype._iterateHierarchy = function(hierarchyObject) {
	if (hierarchyObject.name != null)
		hierarchyObject.label = hierarchyObject.name; 
		
	hierarchyObject.value = hierarchyObject.name; 
	
	//get and set selection value (check box checked or not)
	for (var i = 0; i < this._selections.length; i++) {
		if (this._selections[i].value == hierarchyObject.label) {
			hierarchyObject.selected = true;
		} else {
			hierarchyObject.selected = false;
		}
	}
	
    var selectedHierarchylevelnames = getHierarchylevelnameSelection();
	if (selectedHierarchylevelnames.indexOf(hierarchyObject.label) > -1) {
		hierarchyObject.selected = true;
	}
	
	var selectedScenarios = getScenarioSelection();
	if (selectedScenarios.indexOf(hierarchyObject.label) > -1) {
		hierarchyObject.selected = true;
	} 
	
	//fill labels
	var facetName = this.getLabel();
	if (facetName == hierarchylevelnameFacet && selectedHierarchylevelnames == "-") {
		selectedHierarchylevelnames = hierarchyObject.label;
	} else if (facetName == scenarioFacet && selectedScenarios == "-") {
		selectedScenarios = hierarchyObject.label;
	}	 
	 
	//request db for counts, when deepest elem of tree
	if (hierarchyObject.children != null && hierarchyObject.children.length > 0) {
		
		//expand all items with children -> that user can see child elements
		//this._expanded[hierarchyObject.label] = true;
			
		var count = 0;
		var childArray = hierarchyObject.children;
		var zero = false;
		
		for (var i = 0; i < childArray.length; i++) {//check children
			if (zero) {
				i = 0; 
				zero = false;
			}
			this._iterateHierarchy(childArray[i]);
			var childCount = parseInt(childArray[i].count);
			...
			if (childCount > 0) {				 
				...
				if (i > 0) i--;
					...
					if (childArray[0] != null && childArray[0].children != null)
						i = -1;
					else
						zero = true;
				}
		...
		//reset children, if sth. was deleted
		hierarchyObject.children = childArray;
		
		//sum all counts of children
		hierarchyObject.count = parseInt(count);
	
	} else {
		
	    if (hierarchyObject.label != null) { 
	    	if (facetName == hierarchylevelnameFacet) {  ;
	    		hierarchyObject.count = getHierarchylevelnameCounts(hierarchyObject.label); 

	    	} else if (facetName = scenarioFacet)  	
	    		hierarchyObject.count = getScenarioCounts(hierarchyObject.label); 
	    }
	}
...
Exhibit.HierarchicalFacet.prototype._computeFacet = function(items) {
    ...
    if (facetName == hierarchylevelnameFacet) {
    	...
    	if (resetAllBool) { 
    		obj = getInitValues(hierarchylevelnameFacet);
    	} else {	   	
    	...
    	//if resetAll -> do not send http request, just get inital values
    	if (resetAllBool) { 
    		obj = getInitValues(scenarioFacet);
    	} else {
    		setScenarioCounts(httpGet(countAllScenarios + "/" + selectedHierarchylevelnames + "/" + selectedTopics + "/" + selectedDatatypes + "/" + selectedOrganizations + "/" + getScenarioTreeNames() + "/" + selectedBoundingboxes));    	
    		obj = this._iterateHierarchy(getScenarioTree());
    	}
    	
    	//wenn globalCount == 0 dann speicher obj zwischen
    	if (globalCount == 0) {
    		//@see exhibitOutputFunctions
