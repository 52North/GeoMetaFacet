...
Exhibit._Impl.prototype.configureFromDOM = function(root) {
    ...
    if (controlPanelElmts.length === 0) {
        panel = Exhibit.ControlPanel.createFromDOM(Exhibit.jQuery("<div>").prependTo(document.body), null, uiContext);
...
Exhibit._Impl.prototype._showFocusDialogOnItem = function(itemID) {
    var dom, itemLens;
    dom = Exhibit.jQuery.simileDOM("string", "div", "<div class='exhibit-focusDialog-viewContainer' id='lensContainer'></div><div class='exhibit-focusDialog-controls'><button id='closeButton'>" + Exhibit._("%export.focusDialogBoxCloseButtonLabel") + "</button></div>");
...
Exhibit.FacetUtilities.constructFlowingFacetItem = function(label, count, color, selected, facetHasSelection, onSelect, onSelectOnly, uiContext) {
    ...
    var dom = Exhibit.jQuery.simileDOM("string", "div", ('<div class="exhibit-flowingFacet-value-checkbox">' + Exhibit.jQuery.simileBubble("createTranslucentImageHTML", Exhibit.urlPrefix + (facetHasSelection ? (selected ? "images/black-check.png" : "images/no-check.png") : "images/no-check-no-border.png")) + "</div>") + '<a class="exhibit-flowingFacet-value-link" href="#" id="inner"></a> <span class="exhibit-flowingFacet-value-count">(' + count + ")</span>");
...
Exhibit.FacetUtilities.constructHierarchicalFacetItem = function(label, count, color, selected, hasChildren, expanded, facetHasSelection, onSelect, onSelectOnly, onToggleChildren, uiContext) {
    ...
    var dom = Exhibit.jQuery.simileDOM("string", "div", '<div class="exhibit-facet-value-count">' + count + '</div><div class="exhibit-facet-value-inner" id="inner">' + ('<div class="exhibit-facet-value-checkbox">&#160;' + Exhibit.jQuery.simileBubble("createTranslucentImageHTML", Exhibit.urlPrefix + (facetHasSelection ? (selected ? "images/black-check.png" : "images/no-check.png") : "images/no-check-no-border.png")) + "</div>") + '<a class="exhibit-facet-value-link" href="#" id="link"></a>' + (hasChildren ? ('<a class="exhibit-facet-value-children-toggle" href="#" id="toggle">' + Exhibit.jQuery.simileBubble("createTranslucentImageHTML", Exhibit.urlPrefix + "images/down-arrow.png") + Exhibit.jQuery.simileBubble("createTranslucentImageHTML", Exhibit.urlPrefix + "images/right-arrow.png") + "</a>") : "") + "</div>" + (hasChildren ? '<div class="exhibit-facet-childrenContainer" id="childrenContainer"></div>' : ""));
    Exhibit.jQuery(dom.elmt).attr("class", selected ? "exhibit-facet-value exhibit-facet-value-selected" : "exhibit-facet-value");
    if (typeof label === "string") {
        Exhibit.jQuery(dom.elmt).attr("title", label);
...
Exhibit.FacetUtilities.constructFlowingHierarchicalFacetItem = function(label, count, color, selected, hasChildren, expanded, facetHasSelection, onSelect, onSelectOnly, onToggleChildren, uiContext) {
    ...
    var dom = Exhibit.jQuery.simileDOM("string", "div", ('<div class="exhibit-flowingFacet-value-checkbox">' + Exhibit.jQuery.simileBubble("createTranslucentImageHTML", Exhibit.urlPrefix + (facetHasSelection ? (selected ? "images/black-check.png" : "images/no-check.png") : "images/no-check-no-border.png")) + "</div>") + '<a class="exhibit-flowingFacet-value-link" href="#" id="inner"></a> <span class="exhibit-flowingFacet-value-count">(' + count + ")</span>" + (hasChildren ? ('<a class="exhibit-flowingFacet-value-children-toggle" href="#" id="toggle">' + Exhibit.jQuery.simileBubble("createTranslucentImageHTML", Exhibit.urlPrefix + "images/down-arrow.png") + Exhibit.jQuery.simileBubble("createTranslucentImageHTML", Exhibit.urlPrefix + "images/right-arrow.png") + "</a>") : "") + (hasChildren ? '<div class="exhibit-flowingFacet-childrenContainer" id="childrenContainer"></div>' : ""));
    Exhibit.jQuery(dom.elmt).attr("class", selected ? "exhibit-flowingFacet-value exhibit-flowingFacet-value-selected" : "exhibit-flowingFacet-value");
    if (typeof label === "string") {
        Exhibit.jQuery(dom.elmt).attr("title", label);
...
Exhibit.FacetUtilities.Cache = function(database, collection, expression) {
    ...
    Exhibit.jQuery(collection.getElement()).bind("onRootItemsChanged.exhibit", this._onRootItemsChanged);
};
Exhibit.FacetUtilities.Cache.prototype.dispose = function() {
    Exhibit.jQuery(this._collection.getElement()).unbind("onRootItemsChanged.exhibit", this._onRootItemsChanged);
...
Exhibit.jQuery(document).bind("importReady.exhibit", Exhibit.History.componentStateListener);
Exhibit.Util.HTML = {};
...
Exhibit.Localization.setLocale = function(locales) {
    ...
    Exhibit.jQuery(document).trigger("localeSet.exhibit", [urls]);
};
...
Exhibit.jQuery(document).one("registerLocalization.exhibit", Exhibit.Localization._registerComponent);
Exhibit.jQuery(document).bind("localesRegistered.exhibit", function() {
...
Exhibit.Set.prototype.add = function(o) {
    if ((typeof o === "number" || typeof o === "string" || typeof o === "boolean") && typeof this._hash[o] === "undefined") {
        this._hash[o] = true;
        this._count++;
        return true;
    }
...
Exhibit.ViewUtilities.openBubbleForItems = function(anchorElmt, arrayOfItemIDs, uiContext) {
    ...
    bubble = Exhibit.jQuery.simileBubble("createBubbleForPoint", coords.left + Math.round(anchorElmt.offsetWidth / 2), coords.top + Math.round(anchorElmt.offsetHeight / 2), uiContext.getSetting("bubbleWidth"), uiContext.getSetting("bubbleHeight"));
...
Exhibit.ViewUtilities.fillBubbleWithItems = function(bubbleElmt, arrayOfItemIDs, labelExpression, uiContext) {
    ...
    if (arrayOfItemIDs.length > 1) {
        ...
        makeItem = function(elmt) {
            Exhibit.jQuery("<li>").append(elmt).appendTo(ul);
        };
...
Exhibit.ViewUtilities.constructPlottingViewDom = function(div, uiContext, showSummary, resizableDivWidgetSettings, legendWidgetSettings) {
    var dom = Exhibit.jQuery.simileDOM("string", div, '<div class="exhibit-views-header">' + (showSummary ? '<div id="collectionSummaryDiv"></div>' : "") + '<div id="unplottableMessageDiv" class="exhibit-views-unplottableMessage"></div></div><div id="resizableDiv"></div><div id="legendDiv"></div>', {});
    if (showSummary) {
...
Exhibit.ViewUtilities._setUnplottableMessage = function(dom, totalCount, unplottableItems, uiContext) {
    ...
    if (unplottableItems.length === 0) {
        ...
        dom = Exhibit.jQuery.simileDOM("string", div, Exhibit.ViewUtilities.unplottableMessageFormatter(totalCount, unplottableItems), {});
...
Exhibit.Database._LocalImpl.prototype.loadLinks = function() {
    var links = Exhibit.jQuery("head > link[rel='exhibit-data']").add("head > link[rel='exhibit/data']");
...
Exhibit.Database._LocalImpl.prototype.labelItemsOfType = function(count, typeID, countStyleClass) {
    ...
    span = Exhibit.jQuery("<span>").append(Exhibit.jQuery("<span>").attr("class", countStyleClass).html(count)).append(" " + label);
    return span;
};
...
Exhibit.Collection.prototype._setElement = function(el) {
    if (typeof el === "undefined" || el === null) {
        if (this.getID() !== "default") {
            this._elmt = Exhibit.jQuery("<div>").attr("id", this.getID()).attr(Exhibit.makeExhibitAttribute("role"), "exhibit-collection").css("display", "none").appendTo(document.body).get(0);
        } else {
            this._elmt = document;
        }
    } else {
...
Exhibit.jQuery(document).one("registerStaticComponents.exhibit", Exhibit.Exporter._registerComponent);
...
Exhibit.jQuery(document).one("registerExporters.exhibit", Exhibit.Exporter.BibTex._register);
...
Exhibit.jQuery(document).one("registerExporters.exhibit", Exhibit.Exporter.ExhibitJSON._register);
...
Exhibit.jQuery(document).one("registerExporters.exhibit", Exhibit.Exporter.RDFXML._register);
...
Exhibit.jQuery(document).one("registerExporters.exhibit", Exhibit.Exporter.SemanticWikiText._register);
...
Exhibit.jQuery(document).one("registerExporters.exhibit", Exhibit.Exporter.TSV._register);
...
Exhibit.Importer.prototype._loadURL = function(url, database, callback) {
    ...
    if ((fragmentStart >= 0) && (fragmentStart < url.length - 1)) {
        ...
        callback = function(data, status, jqxhr) {
            var msg, fragment = Exhibit.jQuery(data).find(fragmentId).andSelf().filter(fragmentId);
...
Exhibit.jQuery(document).one("registerStaticComponents.exhibit", Exhibit.Importer._registerComponent);
...
Exhibit.jQuery(document).one("registerImporters.exhibit", Exhibit.Importer.ExhibitJSON._register);
...
Exhibit.Importer.JSONP._register = function(evt, reg) {
    ...
    if (!reg.hasRegistry(Exhibit.Importer.JSONP._registryKey)) {
        ...
        Exhibit.jQuery(document).trigger("registerJSONPImporters.exhibit", reg);
    }
};
Exhibit.jQuery(document).one("registerImporters.exhibit", Exhibit.Importer.JSONP._register);
...
Exhibit.jQuery(document).one("registerJSONPImporters.exhibit", Exhibit.Importer.JSONP.GoogleSpreadsheet._register);
...
Exhibit.jQuery(document).one("registerImporters.exhibit", Exhibit.Importer.BabelBased._register);
...
Exhibit.ControlPanel.mouseOutsideElmt = function(evt, elmt) {
    ...
    return (evt.pageX < coords.left || evt.pageX > coords.left + Exhibit.jQuery(elmt).outerWidth() || evt.pageY < coords.top || evt.pageY > coords.top + Exhibit.jQuery(elmt).outerHeight());
};
...
Exhibit.jQuery(document).one("registerComponents.exhibit", Exhibit.ControlPanel.registerComponent);
...
Exhibit.Formatter.createListDelimiter = function(parentElmt, count, uiContext) {
    ...
    f = function() {
        if (f.index > 0 && f.index < count) {
            if (count > 2) {
                Exhibit.jQuery(parentElmt).append(document.createTextNode((f.index === count - 1) ? lastSeparator : separator));
            } else {
...
Exhibit.Lens.prototype._constructDefaultUI = function(itemID, div, uiContext) {
    ...
    for (j = 0; j < pairs.length; j++) {
        pair = pairs[j];
        tr = Exhibit.jQuery("<tr>").appendTo(dom.propertiesTable);
        tr = Exhibit.jQuery(dom.propertiesTable.get(0).insertRow(j)).attr("class", "exhibit-lens-property");
        Exhibit.jQuery("<td>").appendTo(tr).attr("class", "exhibit-lens-property-name").html(pair.propertyLabel + ": ");
...
Exhibit.Lens._constructFromLensTemplateNode = function(roots, rootValueTypes, templateNode, parentElmt, opts) {
    ...
    if (typeof templateNode.control !== "undefined" && templateNode.control !== null) {
        ...
        case "item-link":
            a = Exhibit.jQuery("<a>").html(Exhibit._("%general.itemLinkLabel")).attr("href", Exhibit.Persistence.getItemLink(itemID)).attr("target", "_blank");
...
Exhibit.UI.makeActionLink = function(text, handler) {
    var a, handler2;
    a = Exhibit.jQuery("<a>" + text + "</a>").attr("href", "#").addClass("exhibit-action");
...
Exhibit.UI.makeItemSpan = function(itemID, label, uiContext) {
    ...
    a = Exhibit.jQuery("<a>" + label + "</a>").attr("href", Exhibit.Persistence.getItemLink(itemID)).addClass("exhibit-item");
    handler = function(evt) {
...
Exhibit.UI.showItemInPopup = function(itemID, elmt, uiContext, opts) {
    ...
    Exhibit.jQuery.simileBubble("createBubbleForContentAndPoint", itemLensDiv, opts.coords.x, opts.coords.y, uiContext.getSetting("bubbleWidth"));
};
Exhibit.UI.createButton = function(name, handler, className) {
    var button = Exhibit.jQuery("<button>").html(name).addClass((className || "exhibit-button")).addClass("screen");
    button.bind("click", handler);
    return button;
};
Exhibit.UI.createPopupMenuDom = function(element) {
    var div, dom;
    div = Exhibit.jQuery("<div>").addClass("exhibit-menu-popup").addClass("exhibit-ui-protection");
    dom = {
        elmt: div,
        open: function(evt) {
        ...
        appendMenuItem: function(label, icon, onClick) {
            ...
            self = this;
            a = Exhibit.jQuery("<a>").attr("href", "#").addClass("exhibit-menu-item").bind("click", function(evt) {
                onClick(evt);
                dom.close();
                evt.preventDefault();
                evt.stopPropagation();
            ...
            a.append(container);
            container.append(Exhibit.jQuery.simileBubble("createTranslucentImage", (typeof icon !== "undefined" && icon !== null) ? icon : (Exhibit.urlPrefix + "images/blank-16x16.png")));
...
Exhibit.UI._clickInElement = function(x, y, elmt) {
    ...
    var dims = {
        w: Exhibit.jQuery(elmt).outerWidth(),
...
Exhibit.jQuery(document).one("registerComponents.exhibit", Exhibit.Facet.registerComponent);
...
Exhibit.ListFacet.prototype._computeFacet = function(items) {
    ...
    if (this._settings.showMissing || this._selectMissing) {
        ...
        if (count > 0 || this._selectMissing) {
            span = Exhibit.jQuery("<span>").attr("class", "exhibit-facet-value-missingThisField").html((typeof this._settings.missingLabel !== "undefined") ? this._settings.missingLabel : Exhibit._("%facets.missingThisField"));
            entries.unshift({
                value: null,
                count: count,
...
Exhibit.CloudFacet.prototype._computeFacet = function(items) {
    ...
    if (this._settings.showMissing || this._selectMissing) {
        ...
        if (count > 0 || this._selectMissing) {
            ...
            Exhibit.jQuery(span).html((typeof this._settings.missingLabel !== "undefined") ? this._settings.missingLabel : Exhibit._("%facets..missingThisField"));
...
Exhibit.CloudFacet.prototype._initializeUI = function() {
    ...
    var dom = Exhibit.jQuery.simileDOM("string", this.getContainer(), ((typeof this._settings.facetLabel !== "undefined") ? ("<div class='exhibit-cloudFacet-header'><span class='exhibit-cloudFacet-header-title'>" + this.getLabel() + "</span></div>") : "") + '<div class="exhibit-cloudFacet-body" id="valuesContainer"></div>');
    this._dom = dom;
};
Exhibit.CloudFacet.prototype._constructBody = function(entries) {
    ...
    if (entries.length > 0) {
        ...
        constructValue = function(entry) {
            ...
            Exhibit.jQuery(elmt).append(document.createTextNode("\u00A0"));
            Exhibit.jQuery(elmt).attr("class", entry.selected ? "exhibit-cloudFacet-value exhibit-cloudFacet-value-selected" : "exhibit-cloudFacet-value");
            if (entry.count > min) {
...
Exhibit.NumericRangeFacet = function(containerElmt, uiContext) {
    var self = this;
    Exhibit.jQuery.extend(this, new Exhibit.Facet("numericrange", containerElmt, uiContext));
    ...
    Exhibit.jQuery(uiContext.getCollection().getElement()).bind("onRootItemsChanged.exhibit", this._onRootItemsChanged);
};
...
Exhibit.NumericRangeFacet.prototype._dispose = function() {
    Exhibit.jQuery(this.getUIContext().getCollection().getElement()).unbind("onRootItemsChanged.exhibit", this._onRootItemsChanged);
    this._dom = null;
    this._ranges = null;
    this._rangeIndex = null;
};
...
Exhibit.TextSearchFacet.prototype.hasRestrictions = function() {
    return this._text !== null; //-> Wenn Texteingabe dann true
};
...
Exhibit.TextSearchFacet.prototype.restrict = function(items) {
    ...
    if (this._text === null) {
        ...
        Exhibit.jQuery(this.getContainer()).trigger("onTextSearchFacetSearch.exhibit", [this._text]);
        this._buildMaps();
        set = new Exhibit.Set();
        itemToValue = this._itemToValue;
...
Exhibit.TextSearchFacet.constructFacetFrame = function(div, facetLabel) {
    if (typeof facetLabel !== "undefined" && facetLabel !== null && facetLabel !== "") {
        return Exhibit.jQuery.simileDOM("string", div, '<div class="exhibit-facet-header"><span class="exhibit-facet-header-title">' + facetLabel + '</span></div><div class="exhibit-text-facet"><input type="text" id="input"></div>');
    } else {
        return Exhibit.jQuery.simileDOM("string", div, '<div class="exhibit-text-facet"><input type="text" id="input"></div>');
    }
};
...
Exhibit.HierarchicalFacet.prototype.update = function(items) {
    var tree;
    Exhibit.jQuery(this._dom.valuesContainer).hide().empty();
    tree = this._computeFacet(items); //3
...
Exhibit.HierarchicalFacet.prototype._computeFacet = function(items) { //4
...
Exhibit.HierarchicalFacet.prototype._constructBody = function(tree) { //5
...
Exhibit.Coder = function(key, div, uiContext) {
    ...
    this.register = function() {
        this.getUIContext().getMain().getRegistry().register(Exhibit.Coder.getRegistryKey(), this.getID(), this);
        _registered = true;
    };
    this.unregister = function() {
        self.getUIContext().getMain().getRegistry().unregister(Exhibit.Coder.getRegistryKey(), self.getID());
        _registered = false;
    };
    ...
    _setIdentifier = function() {
        _id = Exhibit.jQuery(_div).attr("id");
        if (typeof _id === "undefined" || _id === null) {
            _id = _instanceKey + "-" + self.getUIContext().getCollection().getID() + "-" + self.getUIContext().getMain().getRegistry().generateIdentifier(Exhibit.Coder.getRegistryKey());
        }
    };
    _setIdentifier();
};
...
Exhibit.jQuery(document).one("registerComponents.exhibit", Exhibit.Coder.registerComponent);
Exhibit.ColorCoder = function(containerElmt, uiContext) {
    Exhibit.jQuery.extend(this, new Exhibit.Coder("color", containerElmt, uiContext));
...
Exhibit.ColorCoder.create = function(configuration, uiContext) {
    var coder, div;
    div = Exhibit.jQuery("<div>").hide().appendTo("body");
...
Exhibit.ColorCoder.createFromDOM = function(configElmt, uiContext) {
    ...
    configuration = Exhibit.getConfigurationFromDOM(configElmt);
    coder = new Exhibit.ColorCoder(configElmt, Exhibit.UIContext.create(configuration, uiContext));
    Exhibit.SettingsUtilities.collectSettingsFromDOM(configElmt, coder.getSettingSpecs(), coder._settings);
    try {
        Exhibit.jQuery(configElmt).children().each(function(index, elmt) {
            coder._addEntry(Exhibit.getAttribute(this, "case"), Exhibit.jQuery(this).text().trim(), Exhibit.getAttribute(this, "color"));
        });
    } catch (e) {
...
Exhibit.ColorCoder._configure = function(coder, configuration) {
    var entries, i;
    Exhibit.SettingsUtilities.collectSettings(configuration, coder.getSettingSpecs(), coder._settings);
...
Exhibit.ColorGradientCoder = function(containerElmt, uiContext) {
    Exhibit.jQuery.extend(this, new Exhibit.Coder("colorgradient", containerElmt, uiContext));
...
Exhibit.ColorGradientCoder.create = function(configuration, uiContext) {
    var div, coder;
    div = Exhibit.jQuery("<div>").hide().appendTo("body");
    coder = new Exhibit.ColorGradientCoder(div, Exhibit.UIContext.create(configuration, uiContext));
...
Exhibit.ColorGradientCoder.createFromDOM = function(configElmt, uiContext) {
    ...
    Exhibit.jQuery(configElmt).hide();
    configuration = Exhibit.getConfigurationFromDOM(configElmt);
    coder = new Exhibit.ColorGradientCoder(configElmt, Exhibit.UIContext.create(configuration, uiContext));
    Exhibit.SettingsUtilities.collectSettingsFromDOM(configElmt, coder.getSettingSpecs(), coder._settings);
    try {
...
Exhibit.IconCoder = function(containerElmt, uiContext) {
    Exhibit.jQuery.extend(this, new Exhibit.Coder("icon", containerElmt, uiContext));
...
Exhibit.IconCoder.create = function(configuration, uiContext) {
    var coder, div;
    div = Exhibit.jQuery("<div>").hide().appendTo("body");
    coder = new Exhibit.IconCoder(div, Exhibit.UIContext.create(configuration, uiContext));
...
Exhibit.IconCoder.createFromDOM = function(configElmt, uiContext) {
    ...
    Exhibit.jQuery(configElmt).hide();
    configuration = Exhibit.getConfigurationFromDOM(configElmt);
    coder = new Exhibit.IconCoder(configElmt, Exhibit.UIContext.create(configuration, uiContext));
    Exhibit.SettingsUtilities.collectSettingsFromDOM(configElmt, coder.getSettingSpecs(), coder._settings);
    try {
        Exhibit.jQuery(configElmt).children().each(function(index, elmt) {
            coder._addEntry(Exhibit.getAttribute(this, "case"), Exhibit.jQuery(this).text().trim(), Exhibit.getAttribute(this, "icon"));
        });
    } catch (e) {
...
Exhibit.OrderedColorCoder = function(containerElmt, uiContext) {
    Exhibit.jQuery.extend(this, new Exhibit.Coder("orderedcolor", containerElmt, uiContext));
...
Exhibit.OrderedColorCoder.create = function(configuration, uiContext) {
    var coder, div;
    div = Exhibit.jQuery("<div>").hide().appendTo("body");
    coder = new Exhibit.OrderedColorCoder(div, Exhibit.UIContext.create(configuration, uiContext));
...
Exhibit.OrderedColorCoder.createFromDOM = function(configElmt, uiContext) {
    ...
    Exhibit.jQuery(configElmt).hide();
    configuration = Exhibit.getConfigurationFromDOM(configElmt);
    coder = new Exhibit.OrderedColorCoder(configElmt, Exhibit.UIContext.create(configuration, uiContext));
    Exhibit.SettingsUtilities.collectSettingsFromDOM(configElmt, coder.getSettingSpecs(), coder._settings);
    try {
...
Exhibit.SizeCoder = function(containerElmt, uiContext) {
    Exhibit.jQuery.extend(this, new Exhibit.Coder("size", containerElmt, uiContext));
...
Exhibit.SizeCoder.create = function(configuration, uiContext) {
    var coder, div;
    div = Exhibit.jQuery("<div>").hide().appendTo("body");
    coder = new Exhibit.SizeCoder(div, Exhibit.UIContext.create(configuration, uiContext));
...
Exhibit.SizeCoder.createFromDOM = function(configElmt, uiContext) {
    ...
    Exhibit.jQuery(configElmt).hide();
    configuration = Exhibit.getConfigurationFromDOM(configElmt);
    coder = new Exhibit.SizeCoder(configElmt, Exhibit.UIContext.create(configuration, uiContext));
    Exhibit.SettingsUtilities.collectSettingsFromDOM(configElmt, coder.getSettingSpecs(), coder._settings);
    try {
        Exhibit.jQuery(configElmt).children().each(function(index, elmt) {
            coder._addEntry(Exhibit.getAttribute(this, "case"), Exhibit.jQuery(this).text().trim(), Exhibit.getAttribute(this, "size"));
        });
    } catch (e) {
...
Exhibit.SizeGradientCoder = function(containerElmt, uiContext) {
    Exhibit.jQuery.extend(this, new Exhibit.Coder("sizegradient", containerElmt, uiContext));
...
Exhibit.SizeGradientCoder.create = function(configuration, uiContext) {
    var coder, div;
    div = Exhibit.jQuery("<div>").hide().appendTo("body");
    coder = new Exhibit.SizeGradientCoder(div, Exhibit.UIContext.create(configuration, uiContext));
...
Exhibit.OrderedViewFrame.createFooterDom = function(uiContext, footerDiv, onShowAll, onDontShowAll, gotoPage) {
    var dom;
    dom = Exhibit.jQuery.simileDOM("string", footerDiv, Exhibit.OrderedViewFrame.footerTemplate + '<div class="exhibit-collectionView-pagingControls" style="display: none;" id="bottomPagingDiv"></div>', {});
    ...
    dom.setCounts = function(count, limitCount, showAll, canToggle) {
        ...
        if (canToggle && count > limitCount) {
            ...
            if (showAll) {
                Exhibit.jQuery(dom.showAllSpan).append(Exhibit.UI.makeActionLink(Exhibit._("%orderedViewFrame.formatDontShowAll", limitCount), onDontShowAll));
            } else {
                Exhibit.jQuery(dom.showAllSpan).append(Exhibit.UI.makeActionLink(Exhibit._("%orderedViewFrame.formatShowAll", count), onShowAll));
            }
        }
    };
    dom.renderPageLinks = function(page, totalPage, pageWindow) {
...
Exhibit.OrderedViewFrame.renderPageLinks = function(parentElmt, page, pageCount, pageWindow, gotoPage) {
    ...
    renderPageLink = function(label, index) {
        var elmt, a, handler;
        elmt = Exhibit.jQuery("<span>").attr("class", "exhibit-collectionView-pagingControls-page");
        Exhibit.jQuery(parentElmt).append(elmt);
        a = Exhibit.jQuery("<a>").html(label).attr("href", "#").attr("title", Exhibit.ViewUtilities.makePagingLinkTooltip(index));
        elmt.append(a);
        handler = function(evt) {
            gotoPage(index);
            evt.preventDefault();
    ...
    renderPageNumber = function(index) {
        if (index === page) {
            var elmt = Exhibit.jQuery("<span>").attr("class", "exhibit-collectionView-pagingControls-currentPage").html(index + 1);
            Exhibit.jQuery(parentElmt).append(elmt);
        } else {
    ...
    renderHTML = function(html) {
        var elmt = Exhibit.jQuery("<span>").html(html);
        Exhibit.jQuery(parentElmt).append(elmt);
    };
    if (page > 0) {
...
Exhibit.TabularView = function(containerElmt, uiContext) {
    var view = this;
    Exhibit.jQuery.extend(this, new Exhibit.View("tabular", containerElmt, uiContext));
    this.addSettingSpecs(Exhibit.TabularView._settingSpecs);
    Exhibit.jQuery.extend(this._settings, {
        rowStyler: null,
        tableStyler: null,
        indexMap: {}
    });
...
Exhibit.ThumbnailView = function(containerElmt, uiContext) {
    var view = this;
    Exhibit.jQuery.extend(this, new Exhibit.View("thumbnail", containerElmt, uiContext));
...
Exhibit.TileView = function(containerElmt, uiContext) {
    var view = this;
    Exhibit.jQuery.extend(this, new Exhibit.View("tile", containerElmt, uiContext));
...
Exhibit.ViewPanel.createFromDOM = function(div, uiContext) {
    ...
    Exhibit.jQuery(div).children().each(function(index, elmt) {
        ...
        if (role === "view") {
            viewClass = Exhibit.TileView; //7
            viewClassName = Exhibit.getAttribute(this, "viewClass");
            //console.log("viewClassName :"+viewClassName);
...
Exhibit.ViewPanel.prototype._initializeUI = function() {
    ...
    self = this;
    this._dom = Exhibit.ViewPanel.constructDom(Exhibit.jQuery(this._div).children().get(0), this._viewLabels, this._viewTooltips, function(index) {
...
Exhibit.ViewPanel.prototype._switchView = function(newIndex) {
    Exhibit.jQuery(this.getContainer()).trigger("onBeforeViewPanelSwitch.exhibit", [this._viewIndex]);
    if (this._view !== null) {
        this._view.dispose();
        this._view = null;
    }
    ...
    this._createView();
    Exhibit.jQuery(this.getContainer()).trigger("onAfterViewPanelSwitch.exhibit", [this._viewIndex, this._view]);
};
...
Exhibit.jQuery(document).one("registerComponents.exhibit", Exhibit.ViewPanel._registerComponent);
...
Exhibit.View.addViewState = function(id, state) {
    ...
    if (fullState !== null) {
        if (typeof fullState.data.components[id] === "undefined") {
            ...
            Exhibit.jQuery(document).trigger("importReady.exhibit", [Exhibit.View.getRegistryKey(), id]);
        }
    }
};
Exhibit.jQuery(document).one("registerComponents.exhibit", Exhibit.View.registerComponent);
...
Exhibit.BookmarkWidget.prototype._initializeUI = function() {
    var popup;
    popup = Exhibit.jQuery("<div>").attr("class", "exhibit-bookmarkWidget-popup");
...
Exhibit.BookmarkWidget.prototype._fillPopup = function(popup) {
    ...
    Exhibit.jQuery(img).attr("class", "exhibit-bookmarkWidget-button").attr("title", Exhibit._("%widget.bookmark.tooltip")).bind("click", function(evt) {
...
Exhibit.BookmarkWidget.prototype._showBookmark = function(elmt, evt) {
    ...
    el = Exhibit.jQuery('<input type="text" />').attr("value", Exhibit.Bookmark.generateBookmark()).attr("size", 40);
...
Exhibit.CollectionSummaryWidget = function(containerElmt, uiContext) {
    ...
    Exhibit.jQuery(this._collection.getElement()).bind("onItemsChanged.exhibit", this._onItemsChanged);
};
...
Exhibit.CollectionSummaryWidget.prototype.dispose = function() {
    Exhibit.jQuery(this._uiContext.getCollection().getElement()).unbind("onItemsChanged.exhibit", this._onItemsChanged);
    Exhibit.jQuery(this._div).empty();
...
Exhibit.CollectionSummaryWidget.prototype._initializeUI = function() {
    ...
    Exhibit.jQuery(this._div).hide();
    this._allResultsDom = Exhibit.jQuery.simileDOM("string", "span", Exhibit._("%widget.collectionSummary.allResultsTemplate", "exhibit-collectionSummaryWidget-results"));
    this._filteredResultsDom = Exhibit.jQuery.simileDOM("string", "span", Exhibit._("%widget.collectionSummary.filteredResultsTemplate", "exhibit-collectionSummaryWidget-results"), {
        resetActionLink: Exhibit.UI.makeActionLink(Exhibit._("%widget.collectionSummary.resetFiltersLabel"), onClearFilters)
    });
    this._noResultsDom = Exhibit.jQuery.simileDOM("string", "span", Exhibit._("%widget.collectionSummary.noResultsTemplate", "exhibit-collectionSummaryWidget-results", "exhibit-collectionSummaryWidget-count"), {
        resetActionLink: Exhibit.UI.makeActionLink(Exhibit._("%widget.collectionSummary.resetFiltersLabel"), onClearFilters)
    });
...
Exhibit.LegendGradientWidget.prototype.addGradient = function(configuration) {
    ...
    for (i = 0; i < configuration.length; i++) {
        ...
        Exhibit.jQuery("<td>").css("background-color", this._makeRGB(configuration[i])).bind("mouseover", function(evt) {
            Exhibit.jQuery(this).css("border", "1.2px solid");
        }).bind("mouseout", function(evt) {
            Exhibit.jQuery(this).css("border", "none");
        }).appendTo(row2);
        Exhibit.jQuery("<td><div>" + configuration[i].value + "</div></td>").appendTo(row3);
        for (j = configuration[i].value + stepSize; i < configuration.length - 1 && j < configuration[i + 1].value; j += stepSize) {
            fraction = (j - configuration[i].value) / (configuration[i + 1].value - configuration[i].value);
            Exhibit.jQuery("<td>").data("count", counter).css("visibility", "hidden").css("overflow", "hidden").append(Exhibit.jQuery("<div>").append(Math.floor(j * 100) / 100).width(2)).appendTo(row1);
            Exhibit.jQuery("<td>").data("count", counter).css("background-color", this._makeRGB({
                ...
                }).css("visibility", "visible").css("overflow", "visible");
                Exhibit.jQuery(this).css("border", "1.2px solid");
            }).bind("mouseout", function(evt) {
                var self = Exhibit.jQuery(this);
                Exhibit.jQuery(row1).find("td").filter(function(i, el) {
                    return Exhibit.jQuery(el).data("count") === self.data("count");
                }).css("visibility", "hidden").css("overflow", "hidden");
                Exhibit.jQuery(this).css("border", "none");
            }).appendTo(row2);
...
Exhibit.LegendGradientWidget.prototype.addEntry = function(color, label) {
    Exhibit.jQuery("<td>").width("1.5em").height("2em").appendTo(this._row1);
    Exhibit.jQuery("<td>").appendTo(this._row1);
    Exhibit.jQuery("<td>").appendTo(this._row2);
    Exhibit.jQuery("<td>").appendTo(this._row3);
    Exhibit.jQuery("<td>").css("background-color", color).appendTo(this._row2);
    Exhibit.jQuery("<td>").append("<div>" + label + "</div>").appendTo(this._row3);
};
Exhibit.LegendGradientWidget.prototype.addLegendLabel = function(label) {
    var dom;
    dom = Exhibit.jQuery.simileDOM("string", "div", '<div class="legend-label"><span class="label" class="exhibit-legendWidget-entry-title">' + label.replace(/\s+/g, "&nbsp;") + "</span>&nbsp;&nbsp; </div>", {});
...
Exhibit.LegendGradientWidget.prototype._initializeUI = function() {
    ...
    table = Exhibit.jQuery("<table>").width("80%").css("margin-left", "auto").css("margin-right", "auto").css("empty-cells", "show").attr("cellspacing", 0);
    tbody = Exhibit.jQuery("<tbody>").appendTo(table);
    row1 = Exhibit.jQuery("<tr>").height("2em").appendTo(tbody);
    row2 = Exhibit.jQuery("<tr>").height("2em").appendTo(tbody);
    row3 = Exhibit.jQuery("<tr>").height("2em").appendTo(tbody);
    this._row1 = row1;
    this._row2 = row2;
    this._row3 = row3;
...
Exhibit.LegendWidget.prototype.addEntry = function(value, label, type) {
    ...
    if (type === "color") {
        dom = Exhibit.jQuery.simileDOM("string", "span", '<span id="marker"></span>&nbsp;<span id="label" class="exhibit-legendWidget-entry-title">' + label.replace(/\s+/g, "&nbsp;") + "</span>&nbsp;&nbsp; ", {
...
Exhibit.LegendWidget._defaultColorMarkerGenerator = function(value) {
    var span;
    span = Exhibit.jQuery("<span>").attr("class", "exhibit-legendWidget-entry-swatch").css("background", value).html("&nbsp;&nbsp;");
    return span.get(0);
};
Exhibit.LegendWidget._defaultSizeMarkerGenerator = function(value) {
    var span;
    span = Exhibit.jQuery("<span>").attr("class", "exhibit-legendWidget-entry-swatch").height(value).width(value).css("background", "#C0C0C0").html("&nbsp;&nbsp;");
    return span.get(0);
};
Exhibit.LegendWidget._defaultIconMarkerGenerator = function(value) {
    var span;
    span = Exhibit.jQuery("<span>").append('<img src="' + value + '"/>');
    return span.get(0);
};
...
Exhibit.Logo.prototype._initializeUI = function() {
    ...
    a = Exhibit.jQuery("<a>").attr("href", Exhibit.exhibitLink).attr("title", Exhibit.exhibitLink).attr("targe", "_blank").append(img);
...
Exhibit.OptionWidget.prototype._initializeUI = function() {
    ...
    this._dom = Exhibit.jQuery.simileDOM("string", this._containerElmt, sprintf(Exhibit.OptionWidget.uncheckedTemplate + Exhibit.OptionWidget.checkedTemplate, this._label), {
...
Exhibit.ResetHistoryWidget.prototype._initializeUI = function() {
    ...
    Exhibit.jQuery(img).attr("class", "exhibit-resetHistoryWidget-button").attr("title", "Click to clear state and refresh window").bind("click", function(evt) {
...
Exhibit.ResizableDivWidget.prototype._initializeUI = function() {
    var self = this;
    Exhibit.jQuery(this._div).html('<div></div><div class="exhibit-resizableDivWidget-resizer">' + Exhibit.UI.createTranslucentImageHTML("images/down-arrow.png") + "</div>");
    ...
    Exhibit.jQuery(this._resizerDiv).bind("mousedown", function(evt) {
        ...
        self._ondrag = function(evt2) {
            ...
            evt.preventDefault();
            evt.stopPropagation();
            Exhibit.jQuery(self._contentDiv).height(Math.max(height, self._configuration.minHeight));
        };
...
Exhibit.ToolboxWidget.prototype._makePopup = function() {
    ...
    popup = Exhibit.jQuery("<div>").attr("class", "exhibit-toolboxWidget-popup screen").css("position", "absolute").css("top", coords.top + "px").css("right", right + "px");
    this._popup = popup;
    this._fillPopup(popup);
...
Exhibit.ToolboxWidget.prototype._onContainerMouseOver = function(evt) {
    ...
    if (!this._hovering) {
        ...
        coords = Exhibit.jQuery(this._containerElmt).offset();
        docWidth = Exhibit.jQuery(document.body).width();
        popup = Exhibit.jQuery("<div>").hide().attr("class", "exhibit-toolboxWidget-popup screen").css("position", "absolute").css("top", coords.top + "px").css("right", (docWidth - coords.left - Exhibit.jQuery(this._containerElmt).width()) + "px");
        this._fillPopup(popup);
