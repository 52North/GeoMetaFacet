...
function initializeTimeGuiFilling() {
	if (!dijit.byId("fromDate_Input")) {
		dojo.declare("enDateTextBox", dijit.form.DateTextBox, {
			enFormat: {selector:'date', datePattern:'dd-MM-yyyy', locale: 'en-gb'},
			value: "", 
...
function setSliderLabelValues() {
    ...
    var start_DojoDate = dojo.date.locale.format(start_JSDate, {locale:"de-de",datePattern: "dd.MM.yyyy", selector: "date"});
    dojo.byId('time_start_label').innerHTML = start_DojoDate;
    
    var end_JSDate = new Date(time_JSON.end[vis_layer_number]);
    var end_DojoDate = dojo.date.locale.format(end_JSDate, {locale:"de-de",datePattern: "dd.MM.yyyy", selector: "date"});
